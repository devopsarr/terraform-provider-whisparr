		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_notification_twitter.test", "mention", "me"),
					resource.TestCheckResourceAttrSet("whisparr_notification_twitter.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_notification_twitter.test", "mention", "myself"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "whisparr_notification_twitter.test",
				ImportState:       true,
...
func testAccNotificationTwitterResourceConfig(name, mention string) string {
	return fmt.Sprintf(`
	resource "whisparr_notification_twitter" "test" {
		on_grab                            = false
		on_download                        = false
		on_upgrade                         = false

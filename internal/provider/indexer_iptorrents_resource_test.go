		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_indexer_iptorrents.test", "base_url", "https://iptorrents.org"),
					resource.TestCheckResourceAttrSet("whisparr_indexer_iptorrents.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_indexer_iptorrents.test", "base_url", "https://iptorrents.net"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "whisparr_indexer_iptorrents.test",
				ImportState:       true,
...
func testAccIndexerIptorrentsResourceConfig(name, url string) string {
	return fmt.Sprintf(`
	resource "whisparr_indexer_iptorrents" "test" {
		enable_rss = false
		name = "%s"
		base_url = "%s"
		minimum_seeders = 1

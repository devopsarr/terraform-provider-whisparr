		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_metadata_roksbox.test", "movie_metadata", "false"),
					resource.TestCheckResourceAttrSet("whisparr_metadata_roksbox.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_metadata_roksbox.test", "movie_metadata", "true"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "whisparr_metadata_roksbox.test",
				ImportState:       true,
...
func testAccMetadataRoksboxResourceConfig(name, metadata string) string {
	return fmt.Sprintf(`
	resource "whisparr_metadata_roksbox" "test" {
		enable = false
		name = "%s"
		movie_metadata = %s
		movie_images = true

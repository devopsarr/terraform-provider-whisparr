		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_metadata_emby.test", "movie_metadata", "false"),
					resource.TestCheckResourceAttrSet("whisparr_metadata_emby.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("whisparr_metadata_emby.test", "movie_metadata", "true"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "whisparr_metadata_emby.test",
				ImportState:       true,
...
func testAccMetadataEmbyResourceConfig(name, metadata string) string {
	return fmt.Sprintf(`
	resource "whisparr_metadata_emby" "test" {
		enable = false
		name = "%s"
		movie_metadata = %s
	}`, name, metadata)
